#ifndef TREE_SITTER_REASON_SCANNER_H_
#define TREE_SITTER_REASON_SCANNER_H_
...
static bool scan_automatic_semicolon(TSLexer *lexer, bool comment_condition,
                                     bool *scanned_comment) {
...
static inline size_t quoted_string_id_copy(Scanner *scanner, char *buffer) {
  ...
  if (length > 0)
    memcpy(buffer, scanner->quoted_string_id, length);
  return length;
}
...
static bool scan_right_quoted_string_delimiter(Scanner *scanner,
  ...
  for (size_t i = 0; i < scanner->quoted_string_id_length; i++) {
    if (lexer->lookahead != scanner->quoted_string_id[i])
      return false;
    advance(lexer);
  }
...
static bool scan_quoted_string(Scanner *scanner, TSLexer *lexer) {
  if (!scan_left_quoted_string_delimiter(scanner, lexer))
    return false;
  ...
  for (;;) {
    ...
    case '|':
      advance(lexer);
      if (scan_right_quoted_string_delimiter(scanner, lexer))
        return true;
      break;
    case '\0':
      if (eof(lexer))
...
void *tree_sitter_reason_external_scanner_create() { return create(); }
...
unsigned tree_sitter_reason_external_scanner_serialize(void *payload,
                                                       char *buffer) {
  Scanner *scanner = (Scanner *)payload;
  return serialize(scanner, buffer);
}

void tree_sitter_reason_external_scanner_deserialize(void *payload,
                                                     const char *buffer,
                                                     unsigned length) {
  Scanner *scanner = (Scanner *)payload;
...
bool tree_sitter_reason_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[AUTOMATIC_SEMICOLON]) {
    ...
    bool ret = scan_automatic_semicolon(lexer, !valid_symbols[LOGICAL_OR],
                                        &scanned_comment);

    // TODO: Could add this back in
    // if (!ret && !scanned_comment && valid_symbols[TERNARY_QMARK] &&
    //     lexer->lookahead == '?') {
...
#endif // TREE_SITTER_REASON_SCANNER_H_
